    ...
    public class FilesHelper
    {
        ...
        private void UploadWholeFile(HttpContextBase requestContext, List<ViewDataUploadFilesResult> statuses)
        {
            ...
            for (int i = 0; i < request.Files.Count; i++)
            {
                ...
                String newFileName = GuidComb.GenerateComb().ToString() + "." + extansion;
                String pathOnServer = Path.Combine(StorageRoot);
                var fullPath = Path.Combine(pathOnServer, Path.GetFileName(newFileName));
                ...
                if (imageArray.Length != 0)
                {
                    ...
                    else
                    {
                        ...
                        //String fileThumb = Path.GetFileNameWithoutExtension(newFileName);
                        var ThumbfullPath2 = Path.Combine(ThumbfullPath, newFileName);
        ...
        public String CheckThumb(String type, String newFileName)
        {
            ...
            if (splited.Length == 2)
            {
                ...
                {
                    String thumbnailUrl = UrlBase + "thumbs/" + newFileName;
                    return thumbnailUrl;
                }
                else
                {
            ...
            {
                return UrlBase + "/thumbs/" + Path.GetFileNameWithoutExtension(newFileName);
            }

        }

        public List<ViewDataUploadFilesResult> FilesList(Guid id)
        {
            ...
            {
                DirectoryInfo di = new DirectoryInfo(StorageRoot);
